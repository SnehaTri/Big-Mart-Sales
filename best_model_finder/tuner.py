    ...
    def get_best_params_for_linearReg(self,train_x,train_y):
        ...
        try:
            ...
            self.param_grid_linearReg = {
                'fit_intercept': [True, False], 'positive': [True, False], 'copy_X': [True, False], 'n_jobs': [1,2,3]
            ...
            self.positive = self.grid.best_params_['positive']
            self.copy_X = self.grid.best_params_['copy_X']
            self.n_jobs = self.grid.best_params_['n_jobs']

            # creating a new model with the best parameters
            self.linReg = LinearRegression(fit_intercept=self.fit_intercept,positive=self.positive,copy_X=self.copy_X,n_jobs=self.n_jobs)
